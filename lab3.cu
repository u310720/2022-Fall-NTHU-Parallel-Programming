...
__global__ void sobel (unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels) {
    int  x, y, z, i, v, u;
    int color[COLOR];
    double val[MASK_N*COLOR] = {0.0};
    int adjustX, adjustY, xBound, yBound;

    /* Hint 6 */
    // parallel job by blockIdx, blockDim, threadIdx 
    x = threadIdx.x + blockIdx.x * blockDim.x; // width
    y = threadIdx.y + blockIdx.y * blockDim.y; // height
    z = threadIdx.z + blockIdx.z * blockDim.z; // color
    if (x >= width || y >= height || z >= COLOR)
        return;
    
    for (i = 0; i < MASK_N; ++i) {
        adjustX = (MASK_X & 0x01) ? 1 : 0;
        adjustY = (MASK_Y & 0x01) ? 1 : 0;
        xBound = MASK_X >> 1;
        yBound = MASK_Y >> 1;
        ...
        for (v = -yBound; v < yBound + adjustY; ++v) {
            for (u = -xBound; u < xBound + adjustX; ++u) {
                if ((x + u) >= 0 && (x + u) < width && y + v >= 0 && y + v < height) {
                    color[z] = s[channels * (width * (y+v) + (x+u)) + z];
                    val[i*COLOR+z] += color[z] * mask[i][u + xBound][v + yBound];
                }    
            }
        }
    }

    double color_sum[COLOR] = {0.0};
    for (i = 0; i < MASK_N; ++i) {
        color_sum[z] += val[i*COLOR + z] * val[i*COLOR + z];
    }

    color_sum[z] = sqrt(color_sum[z]) / SCALE;
    t[channels * (width * y + x) + z] = color_sum[z] > 255.0 ? 255 : color_sum[z];
}
...
int main(int argc, char** argv) {
    ...
    const unsigned image_size = height * width * channels;
    unsigned char* host_t = (unsigned char*) malloc(image_size * sizeof(unsigned char));
    
    /* Hint 1 */
    // cudaMalloc(...) for device src and device dst
    unsigned char *device_s = NULL;
    unsigned char *device_t = NULL;
    cudaMalloc((void **)&device_s, image_size * sizeof(unsigned char));
    cudaMalloc((void **)&device_t, image_size * sizeof(unsigned char));

    /* Hint 2 */
    // cudaMemcpy(...) copy source image to device (filter matrix if necessary)
    cudaMemcpy(device_s, host_s, image_size * sizeof(unsigned char), cudaMemcpyHostToDevice);

    /* Hint 3 */
    // acclerate this function
    // sobel(host_s, host_t, height, width, channels);
    printf("width=%d, height=%d, channels=%d\n", width, height, channels);
    dim3 dimGrid(ceil((double)width / BLK_WIDTH), ceil((double)height / BLK_HEIGHT), COLOR);
    dim3 dimBlock(BLK_WIDTH, BLK_HEIGHT, COLOR);
    sobel<<<dimGrid, dimBlock>>>(device_s, device_t, height, width, channels);
    
    /* Hint 4 */
    // cudaMemcpy(...) copy result image to host
    cudaMemcpy(host_t, device_t, image_size * sizeof(unsigned char), cudaMemcpyDeviceToHost);
    write_png(argv[2], host_t, height, width, channels);

    cudaFree((void *)device_s);
    cudaFree((void *)device_t);
    free(host_s);
    free(host_t);
    return 0;
}
