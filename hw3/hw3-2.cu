...
struct HostData
{
    ...
    HostData(int nVertex, int nEdge) : nV(nVertex), nE(nEdge)
    {
        ...
        H_Dist = (int *)malloc(nPadV * nPadV * sizeof(int));
        // cudaMallocHost(&(hData->H_Dist), hData->nPadV * hData->nPadV * sizeof(int));
    }
    ~HostData()
    {
        free(H_Dist);
...
/* APSP */
void blk_FW(int *D_Dist, int nV, int nPadV, int *H_Dist);
...
__global__ void calPhase2(int *D_Dist, int pivot, int nPadV);
__global__ void calPhase3(int *D_Dist, int pivot, int nPadV);
...
int main(int argc, char *argv[])
{
    ...
    /* cuda begin */
    const int nPadV = hData->nPadV;
    int *H_Dist = hData->H_Dist;
    int *D_Dist = NULL;
    cudaMalloc(&D_Dist, nPadV * nPadV * sizeof(int));
    cudaMemcpy(D_Dist, H_Dist, nPadV * nPadV * sizeof(int), cudaMemcpyHostToDevice);

    /* blocked Floyd-Washall */
    // h_printMatrix(H_Dist, nPadV); // debug
    blk_FW(D_Dist, hData->nV, hData->nPadV, hData->H_Dist);
    ...
    /* cuda end */
    cudaMemcpy(H_Dist, D_Dist, nPadV * nPadV * sizeof(int), cudaMemcpyDeviceToHost);
    cudaFree(D_Dist);
...
void blk_FW(int *D_Dist, int nV, int nPadV, int *H_Dist)
{
    ...
    {
        // printf("\n\n[pivot]=%d, [phase]=1\n", pivot);                                    // debug
        calPhase1<<<dimGridPhase1, dimBlk>>>(D_Dist, pivot, nPadV);
        // cudaMemcpy(H_Dist, D_Dist, nPadV * nPadV * sizeof(int), cudaMemcpyDeviceToHost); // debug
        // h_printMatrix(H_Dist, nPadV);                                                    // debug

        // printf("\n\n[pivot]=%d, [phase]=2\n", pivot);                                    // debug
        calPhase2<<<dimGridPhase2, dimBlk>>>(D_Dist, pivot, nPadV);
        // cudaMemcpy(H_Dist, D_Dist, nPadV * nPadV * sizeof(int), cudaMemcpyDeviceToHost); // debug
        // h_printMatrix(H_Dist, nPadV);                                                    // debug

        // printf("\n\n[pivot]=%d, [phase]=3\n", pivot);                                    // debug
        calPhase3<<<dimGridPhase3, dimBlk>>>(D_Dist, pivot, nPadV);
        // cudaMemcpy(H_Dist, D_Dist, nPadV * nPadV * sizeof(int), cudaMemcpyDeviceToHost); // debug
        // h_printMatrix(H_Dist, nPadV);                                                    // debug
    }
}
...
{
    /* phase 1, 2 */
    if (blockIdx.x == pivot || blockIdx.y == pivot)
        return;
